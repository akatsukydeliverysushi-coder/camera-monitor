...
function ffmpegAvailable(){return new Promise(resolve=>execFile('ffmpeg',['-version'],{windowsHide:true},e=>resolve(!e)));}
...
function startStream(rtsp){
 return new Promise(async(resolve,reject)=>{
  ...
  if(!(await ffmpegAvailable()))return reject(new Error('FFmpeg não encontrado no PC'));
...
const server=http.createServer(async(req,res)=>{
 ...
 if(req.method==='GET'&&req.url==='/api/health')return json(res,200,{ok:true,service:'Camera Monitor Companion',port:PORT,ffmpeg:'verificando'});
